...
style_img1 = image_to_Tenser("./Image_mini/style1.jpg")
style_img2 = image_to_Tenser("./Image_mini/style2.jpg")
content_img = image_to_Tenser("./Image_mini/dancing.jpg")
...
def blend_images(images, weights):
    """
    :params: images (list of torch.Tensor): 包含要融合的图像张量的列表
    :params: weights (list of float): 对应每个图像的权重
    :return: torch.Tensor: 融合后的图像张量
    """
    if len(images) != len(weights):
        raise ValueError("图片数量必须和权重数量相同")
    total_weight = sum(weights)
    normalized_weights = [w / total_weight for w in weights]
    blended_image = torch.zeros_like(images[0])
    for img, weight in zip(images, normalized_weights):
        blended_image += img * weight
    return blended_image

output1 = run_style_transfer(inception, cnn_normalization_mean, cnn_normalization_std, content_img, style_img1, input_img, 600)
output2 = run_style_transfer(inception, cnn_normalization_mean, cnn_normalization_std, content_img, style_img2, input_img, 300)

weights = [0.8, 0.2]  
output = blend_images([output1, output2], weights)
